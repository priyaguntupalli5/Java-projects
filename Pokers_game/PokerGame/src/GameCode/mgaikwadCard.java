...
public class mgaikwadCard {
	...
	private static mgaikwadCard[] allCards; // an array of all cards in the deck
	...
	private static HashMap<String, mgaikwadCard> cardMap = new HashMap<String, mgaikwadCard>(); // mapping from String representations of cards to Card objects

	static {
		// upon loading the Card class, initialize 52-card French deck cards
		
		// create all card objects
		allCards = new mgaikwadCard[rankNames.length * suitNames.length];
		int i = 0;
		for (int suit = 0; suit < suitNames.length; suit++) 
			for (int rank = 0; rank < rankNames.length; rank++)
				allCards[i++] = new mgaikwadCard(rank, suit);
		
		// create mapping from String representations to Card objects
		for (mgaikwadCard card : allCards)
	 ...
	 */
	public static mgaikwadCard getCard(String cardString) {
	 ...
	 */
	public static mgaikwadCard getCard(int cardId) {
	 ...
	 */
	public static mgaikwadCard[] getAllCards() {
	 ...
	 */
	private mgaikwadCard(int rank, int suit) {
		this.rank = rank;
		this.suit = suit;
	}
	 ...
	 * @return whether or not this card equals another Card object
	 */
	public boolean equals(mgaikwadCard other) {
